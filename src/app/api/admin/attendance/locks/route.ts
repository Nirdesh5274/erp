...
export async function PATCH(request: Request) {
  try {
    ...
    const { data: lecture, error: lectureError } = await supabase
      .from("lectures")
      .select("id,college_id,department_id,attendance_locked,attendance_lock_expires_at")
      .eq("id", body.lectureId)
      .eq("college_id", ctx.collegeId)
      .single();

    if (lectureError) return apiError(lectureError.message, 404);
    if (ctx.role === "HOD" && ctx.departmentId && lecture.department_id !== ctx.departmentId) {
      return apiError("HOD can only update their department", 403);
    }
